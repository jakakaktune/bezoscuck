...
const proxy = function({server, host, is_secure, req_headers, req_options, hooks, cache_segments, max_segments, cache_timeout, cache_key, debug_level, acl_whitelist}) {
  ...
  server.on('request', async (req, res) => {
    ...
    if (req.url.includes('/get-link/')) {
      console.log(req.url.split('/get-link/')[1]);
      let coded_link = req.url.split('/get-link/')[1];
      regexs.wrap.test(req.url)
      let decoded_url = base64_decode( coded_link.replace(regexs.wrap, '$1') ).trim();

      let final_link = "";

      if (decoded_url.includes('/videos/')) {
        let id = decoded_url.split('twitch.tv/videos/')[1];
        if (!id) {
          res.setHeader('Access-Control-Allow-Origin',      '*')
          res.writeHead(400);
          res.end('bad link');
        ...
        final_link = await twitch.getVod(id);
      } else {
        let id = decoded_url.split('twitch.tv/')[1];
        console.log(id);
        if (!id) {
          res.setHeader('Access-Control-Allow-Origin',      '*')
          res.writeHead(400);
        ...
        try {
          final_link = await twitch.getStream(id);
        } catch (error) {
          console.log(error);
        }
      }
